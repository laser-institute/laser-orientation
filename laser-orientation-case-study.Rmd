...
holistic setting to explore important foundational topics integral to
Learning Analytics such as reproducible research, use of APIs, and
...
deep into it for now - we'll be spending more time on this throughout
the week; just know that this document and all of our LASER Lab case
studies are organized around these five components.
...
In this case study, we'll be working with data from a research study,
which utilized a number of different data sources to understand
students' course-related motivation [@estrellado2020e]. For our
purposes, our analysis will be driven by a simple question
...
Since we are working in Posit Cloud with an R project [cloned from
GitHub](https://github.com/laser-institute/laser-orientation), a project
has already been set up for you as indicated by the `.Rproj` file in the
main directory. Locate the Files tab lower right hand window pane and
see if you can find this file.

With a project already set up for us, we will instead focus on loading
...
**Note**: You can also enlarge this output by clocking the "Show in New
...
operator but the original pipe operator `%>%` works just as well.

Run the following code chunk to `select()` the `student_id`,
