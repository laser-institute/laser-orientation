...
::: callout-note
You may have noticed a special kind of link in the text above. Specifically, a link citing Reproducible Research with R and R Studio by Chris Gandrud. The YAML header includes a bibliography option and points to our `reference.bib` file in the `lit` folder of this project. This will produce a nice tooltip for each linked reference as well as a bibliography when our doc is [rendered](https://quarto.org/docs/get-started/hello/rstudio.html#rendering) and [published](https://quarto.org/docs/get-started/authoring/rstudio.html#publishing). If you'd like to learn more about citations in Quarto visit: <https://quarto.org/docs/authoring/citations.html>
:::
...
💡 **Tip:** type the name of the function after the name of the dataset, using `.` before and `()` after it.
...
💡 **Tip:** you can use one function and then another function like `data.function_1().function_2()`
...
#Display the predicted grades
predicted_grades
```

Let's test our model and see the prediction of the grade for 4000 minutes spent studying:
...
2.  Next, click the knit button in the toolbar above to ![](img/render.png){width="10%"} your Markdown document to an [HTML](https://bookdown.org/yihui/rmarkdown/html-document.html) file that will be saved in your Python Project folder. You should see a formatted webpage appear in your Viewer tab in the lower right pan or in a new browser window. Let's us know if you run into any issues with rendering.
...
1.  Congratulations, you've completed your first case study and earned your first official LASER Badge!
