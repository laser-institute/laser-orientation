...
**Pro tip**: Quarto documents also a have handy Outline feature that allow you to easily navigate the entire document. If the outline is not currently visible, click the Outline button located on the right of the toolbar at the top of this document.
:::
...
# add a comment here and your code below this comment
...
```{r}
sci_data |> 
  select(FinalGradeCEMS, TimeSpent, "NEW VARIABLE HERE")
```
...
Run the next chunk of code, imagining that we wish to filter our data to view only the rows associated with students who earned a final grade (as a percentage) of 70 - 70% - or higher.
...
ggplot(data = sci_data, aes(x = TimeSpent, y = FinalGradeCEMS)) +
  ...
  labs(x = 'TimeSpent', y = 'FinalGradeCEMS') +  # Add axis labels
