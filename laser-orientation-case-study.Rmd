...
provider designed to supplement---but not replace---students\'
enrollment in their local school. The data used in this case study has
already been "wrangled" quite a bit, but the original datasets included:

1.  A self-report survey assessing three aspects of students\'
    motivation
...
analysis. In this section, we'll create a few plots to explore our data.
...
this case for students' final grades. Go ahead and run it:
...
```{r, eval=FALSE}
ggplot(sci_data, aes(x = TimeSpent, y = total_points_earned)) +
  geom_point()
```
